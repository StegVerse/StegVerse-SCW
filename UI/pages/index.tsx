...
export default function Home() {
  ...
  return (
    <div style={{maxWidth: 800, margin: "40px auto", fontFamily: "system-ui"}}>
      ...
      <p>API URL: <input value={apiUrl} onChange={e => setApiUrl(e.target.value)} placeholder="https://scw.yourdomain.com" style={{width:"100%"}}/></p>
      ...
      <p><button onClick={run} disabled={!projectId || !apiUrl}>Run</button></p>
      {runId && <p>Run ID: {runId}</p>}
      {result && (
        <div style={{background:"#111", color:"#0f0", padding:10, whiteSpace:"pre-wrap"}}>
          <strong>Status:</strong> {result.status}<br/>
          {result.logs && result.logs.map((line: string, i: number) => <div key={i}>{line}</div>)}
          {result.result && <div>Result: {result.result}</div>}
        </div>
      )}
      <p style={{marginTop:30, fontSize:12, color:"#666"}}>MVP: This simulates execution. Add sandbox containers later.</p>
    </div>
  );
}